...
const faqs = [
  ...
  { q: "What do I need to bring?", a: "This is a strict BYOD (Bring Your Own Device) event. You MUST bring a laptop to actively build your prototype." },
  ...
  return (
    <section id="faq" ref={containerRef} className="py-32 md:py-48 px-6 md:px-12 w-full max-w-4xl mx-auto">
      ...
      <div className="flex flex-col border-t border-white/10">
        {faqs.map((faq, idx) => (
          <div key={idx} className="faq-item border-b border-white/10">
            <button 
              onClick={() => toggle(idx)}
              className="w-full py-6 flex justify-between items-center text-left hover:bg-white/5 transition-colors px-4 group"
            >
              <h3 className="font-poppins text-lg text-white/90 group-hover:text-white transition-colors pr-8">
                {faq.q}
              </h3>
              <ChevronDown 
                className={`w-5 h-5 text-white/50 transition-transform duration-300 ${openIdx === idx ? 'rotate-180' : ''}`} 
              />
            </button>
            <div 
              className={`overflow-hidden transition-all duration-500 ease-in-out px-4 ${openIdx === idx ? 'max-h-40 pb-6 opacity-100' : 'max-h-0 opacity-0'}`}
            >
